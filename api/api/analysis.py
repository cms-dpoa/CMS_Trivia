...
def get_votes(dataset):
    ...
    votes_score_df = (
        votes_df.groupby(["labels"])
        .apply(lambda x: (x["knowledgeLevel"] * x["player_score"] / 25).sum())
        .reset_index()
    )
...
def get_votes_dataset_by_categories(idDataset, label_1, label_2, label_3, label_4):
    labels = {label_1[0]: 0, label_2[0]: 0, label_3[0]: 0, label_4[0]: 0}
