...
def add_labels_to_DB(cursor):
    ...
    for i in range(len(ids)):
        command = (
            f"INSERT INTO api_label VALUES({ids[i]}, '{names[i]}', {was_created[i]}, 1)"
        )
...
def add_datasets_to_DB(cursor):
    ...
    for i in range(len(id_dataset)):
        query_get_label = (
            f"Select * from api_label where id_label = {original_label[i]}"
        )
        cursor.execute(query_get_label)
        id_label = cursor.fetchone()[0]

        query_add_dataset = (
            "INSERT INTO api_data (id_dataset, title, original_label_id, year) VALUES (%d,'%s',%d,%d)"
            % (int(id_dataset[i]), title[i], id_label, int(year[i]))
        )
